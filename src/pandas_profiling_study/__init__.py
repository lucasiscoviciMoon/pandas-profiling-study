...
from .model.messages import MessageType
from .version import __version__
from .utils.dataframe import rename_index
from .utils.paths import get_config_default, get_config_minimal
...
class ProfileReport(object):
    ...
    def __init__(self, df, minimal=False, config_file: Path = None, sections: list =["overview","variables","interactions","correlations","missing","sample"], **kwargs):
        ...
        disable_progress_bar = not config["progress_bar_main"].get(bool)

        with tqdm(
            total=1, desc="build report structure", disable=disable_progress_bar
        ) as pbar:
            self.report = get_report_structure(
                self.date_start, self.date_end, self.sample, description_set, sections
            )
            self.sections=copy.deepcopy(self.report);
            pbar.update()

    def change_sections(self, sections: list =["overview","variables","interactions","correlations","missing","sample"]):
        if not isinstance(sections,list):
            sections=[sections]
        content=copy.deepcopy(self.sections.content["items"])
        sections=[i for i in content if i.anchor_id.lower() in sections]
        self.report.content["items"]=sections
        # self.report = self.to_html()
        return self
    ...
    def to_notebook_iframe(self):
        ...
        """
        from .report.presentation.flavours.widget.notebook import (
            get_notebook_iframe,
        )
    ...
    def to_widgets(self):
        ...
        from .report.presentation.flavours import WidgetReport
    ...
    def to_app(self):
        ...
        """
        from .report.presentation.flavours.qt.app import get_app
        from .report.presentation.flavours import QtReport
