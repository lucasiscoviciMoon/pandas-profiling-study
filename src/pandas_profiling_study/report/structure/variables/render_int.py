...
from ....report.formatters import fmt_array
from ....visualisation.plot import mini_histogram, histogram
...
def render_int(summary):
    varid = summary["varid"]
    template_variables = render_common(summary)
    image_format = config["plot"]["image_format"].get(str)

    if summary["min"] >= 0:
        name = "Int number (&Ropf;<sub>&ge;0</sub>)"
    else:
        name = "Int number (&Ropf;)"
